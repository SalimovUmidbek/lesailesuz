...
const Lester = () => {
  ...
  const getResponse = async () => {
    try {
      const response = await axios.get(
        "https://d93a54b77da76ec7.mokky.dev/lester1"
      );
  ...
  return (
    <div id="lesterlar">
      <h1 className="text-4xl pt-5">Lester</h1>
        ...
        {tokens.map((chickens) => (
          ...
          >
            ...
            <div className="flex justify-end mx-[10px] mt-[-10px]">
              <button
                onClick={() => getBasketData(chickens)}
                className="w-[40px] h-[40px] bg-[#fc004a] rounded-[10px] text-white text-[20px] font-800 "
              >
                +
              </button>
            </div>
